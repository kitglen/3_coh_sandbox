class Api::V1::DoctorsController < ApplicationController
  resource_description do
    ...
    version '1'
    formats ['json']
    param :id, Integer, :desc => "V1::Doctor ID", :required => false
    param :doctor, Hash, :desc => "V1::Doctor's parameters for all methods" do
  ...
  # GET /doctors.json
  api :GET, "/v1/doctors.json", "Show all doctors based on a query scope"
  def index
  ...
  # GET /doctors/1.json
  api :GET, "/v1/doctors/:id.json", "Show doctor"
  def show
  ...
  api :GET, "/v1/doctors/new.json", "Instantiate a new doctor"
  def new
    @doctor = V1::Doctor.new
    respond_with @doctor
  end

  # POST /doctors.json
  api :POST, "/v1/doctors.json", "Create a new doctor"
  def create
  ...
  # PUT /doctors/1.json
  api :PUT, "/v1/doctors/:id.json", "Update an existing doctor"
  def update
  ...
  api :DELETE, "/v1/doctors/:id.json", "Delete an existing doctor"
  def destroy
...
protected
  def scope_named(name)
    scope = 'all' unless ::V1::Doctor.respond_to?(name)
    if name == 'all'
      ...
      scope = ::V1::Doctor.count
    else
      scope = ::V1::Doctor.send(name, (params[:query] || {}), {}, params[:offset], params[:limit])
    end
    scope
  end
  ...
  include Api::V1::Custom::DoctorsController
end
